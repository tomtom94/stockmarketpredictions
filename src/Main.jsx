...
const Main = () => {
  ...
  useEffect(() => {
    ...
    if (areAllDataReady) {
      ...
      const { dataNormalized: sampleDataNormalized, dimensionParams: sampleDimensionParams } = normalizeData(sampleDataRaw)
      setSampleData({
  ...
  useEffect(() => {
    setData(Object.entries(stockMarketDataDaily['Time Series (Daily)']).sort((a, b) => new Date(a[0]) - new Date(b[0])))
  ...
  useEffect(() => {
    if (data.length && series.findIndex((serie) => serie.name === 'Stock value') === -1) {
      setSeries([
        ...series,
        {
          type: 'area',
          id: 'dataseries',
          name: 'Stock value',
          data: data.map((serie) => [new Date(serie[0]).getTime(), Number(serie[1]['4. close'])]),
          gapSize: 5,
          tooltip: {
            valueDecimals: 2
          },
  ...
  const splitData = (trainingRange) => {
    ...
    const descStochastic7 = JSON.parse(JSON.stringify(dataStochastic7)).reverse()
    const descStochastic14 = JSON.parse(JSON.stringify(dataStochastic14)).reverse()
    ...
    const chunk = dataRaw.slice(baseIndex, top === 1 ? undefined : baseIndex + 1 + Math.floor((top - bottom) * dataRaw.length))
    return chunk
  }
  ...
  const normalizeData = (dataRaw, params) => {
    ...
    if (!params) {
      dimensionParams = dataPerDimension.map((dimension) => {
        const mean = dimension.reduce((acc, curr) => acc + curr, 0) / dimension.length
        ...
        return {
          ...
          max,
          mean,
          std: Math.sqrt(dimension.map((e) => (e - mean) ** 2).reduce((acc, curr) => acc + curr, 0) / dimension.length)
    ...
    return {
      dataNormalized: createTimeseriesDimensionForRNN(
        dataRaw.map((set) =>
          set.map(
            (e, i) => (e - dimensionParams[i].mean) / (dimensionParams[i].std + epsilon)
  ...
  const unNormalizeData = (dataRaw, dimensionParam) => dataRaw.map((e) => e * dimensionParam.std + dimensionParam.mean)
  ...
  const makePredictions = async () => {
    ...
    try {
      ...
      timeseriesChunks.forEach((chunk, index, array) => {
        const { dataNormalized, dimensionParams } = normalizeData(chunk.map((e) => e[1]))
        ...
        if (nextChunk) {
          ...
          datePredicted = new Date(lastDate).setDate(new Date(lastDate).getDate() + 1)
        }
  ...
  const rebootSeries = () => {
    setInvesting({ start: 1000, end: null })
    const serieIndex = series.findIndex((serie) => serie.name === 'Predicted value')
      ...
      {series.length > 0 && (
        <>
          ...
          <div style={{ margin: '10px 5px' }}>
            <label>
              <span>Define how many periods are needed for the neural network to detect recurrent patterns :</span>
              <input
                size={2}
                value={recurrence}
                disabled={isModelTraining || isPredictionLoading}
                onChange={(event) => {
                  setRecurrence(isNaN(Number(event.target.value)) ? event.target.value : Number(event.target.value))
                }}
              />
              {typeof recurrence !== 'number' && <span>Only use number</span>}
              {typeof recurrence === 'number' && recurrence < 2 && (
                <span>Minimum 2 recurrences in order to normalize inputs with mean and standard derivation</span>
              )}
              {typeof recurrence === 'number' && recurrence > 32 && <span>It may take a lot of time</span>}
            </label>
            <br />
            <button
              onClick={createModel}
            ...
            >
              {isModelTraining ? '1. Model is training' : '1. Create and validate model'}
            </button>{' '}
            <span style={{ marginLeft: 5, fontWeight: 'bold' }}>
              When you click this button you must <u>stay on this page</u> ! Otherwise your computer will put all your browser's work in
              standby.
              <br />
              <u>It's around 4 minutes work just take a coffee</u>, depends your device's power, don't do this with your smartphone ;) it's
              gonna compile batches of {recurrence} periods.
              <br />
              Bigger are the batches, longer it's gonna take time, smoother would be the prediction line.
            </span>
            <br />
            <br />
            <span>
              Define the flag order strategy (you can change it anytime even when the tensorflow model is already compiled, just need to
              click Make predictions again) :
            </span>
            <br />
            <input
            ...
            <label htmlFor="Default">
              Default - The prediction next day is higher than the prediction today (the reverse for sell order)
            </label>
            <br />
            <input
              disabled={isModelTraining || !modelResultTraining || isPredictionLoading || true}
              checked={strategy == 3}
              type="radio"
              id="classic"
              name="flag-strategy"
            ...
            <label htmlFor="classic">
              Classic - The prediction next day is higher than the real value today (the reverse for sell order)
            </label>
            <br />
            <input
              disabled={isModelTraining || !modelResultTraining || isPredictionLoading || true}
            ...
            <label htmlFor="secure">
              Secure - The prediction next day is higher than the prediction today & the prediction next day is higher than the real value
            ...
            {isModelTraining && (
              <p>
                <u>Please stay on the page, and leave your computer do the job ;)</u>
              </p>
            )}
            {isPredictionLoading && (
              <p>
            ...
            {modelLogs.length > 0 && (
              <>
                {investing.end ? (
                  <p>{`You invested ${investing.start}$, you get off the train with ${Math.round(investing.end)}$`}</p>
                ) : (
                  <p>{`You are investing ${investing.start}$, click on Make predictions button`}</p>
                )}

                <HighchartsReact highcharts={Highcharts} options={options2} constructorType={'chart'} />
              </>
            )}
            ...
            <ul>
              <li>Create and validate model button : use training and validation set (80% and 10%).</li>
              <li>
            ...
            {sampleData && (
              ...
              <>
                ...
                <table border={1}>
                  ...
                  <tbody>
                    {Object.entries(sampleData.sampleDimensionParams).map((e1, i1) => (
                      <tr key={`sampleDimensionParams-row-${i1}`}>
                        {Object.values(e1[1]).map((e2, i2) => (
                          <td key={`sampleDimensionParams-column-${i2}`}>{e2}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
